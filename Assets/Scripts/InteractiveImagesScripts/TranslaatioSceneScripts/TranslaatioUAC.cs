...
public class TranslaatioUAC : MonoBehaviour
{
    [SerializeField]
...
//        go.GetComponent<Text>().text = "Rakenna RNA";
    ...
    private void OnMouseUp() {
            ...
            Mathf.Abs(transform.position.y - objectPlace.position.y) <= 0.5f)
        {
            ...
            GameObject go = GameObject.Find("PlaceholderUAC");
            go.transform.position = new Vector3(-100f,-100f,0);

            GameObject go2 = GameObject.Find("PlaceholderUGA");
            go2.transform.position = new Vector3(gameObject.transform.position.x + 1.5f,
